...
BarBlock {
   ...
   function updateVolume() {
    if (!sink?.audio || typeof sink.audio.volume !== "number") {
        content.symbolText = "󰖁 0%"
        return
    }

    const vol = Math.max(0, Math.min(1, sink.audio.volume))
    const icon = sink.audio.muted ? "󰖁" : "󰕾"
    content.symbolText = `${icon} ${Math.round(vol * 100)}%`
}
    content: BarText {
    symbolText: {
        ...
        const icon = sink?.audio?.muted ? "󰖁" : "󰕾"
        return `${icon} ${Math.round(safeVol * 100)}%`
    }
}
    MouseArea {
        anchors.fill: parent
